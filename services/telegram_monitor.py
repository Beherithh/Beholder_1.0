...
class TelegramMonitorService:
    ...
    async def _scan_messages(self, session, tg_conf) -> int:
        ...
        Использует connect() вместо start(), чтобы избежать интерактивного ввода.
        """
        from pyrogram import Client
        ...
        try:
            # Используем connect() вместо start(), чтобы не триггерить ввод телефона
            await app.connect()
        except Exception as e:
            logger.error(f"[BINANCE-TG] Failed to connect: {e}")
            return 0

        try:
            # Проверяем авторизацию. Если сессия битая, get_me() упадет, но не зависнет.
            try:
                await app.get_me()
            except Exception:
                logger.warning("[BINANCE-TG] Session is invalid or unauthorized. Please re-create session in Settings.")
                return 0
