...
class MarketDataFetcher:
    ...
    async def get_market_info(self, symbol: str = DEFAULT_SYMBOL, market_type: MarketType = MarketType.SPOT) -> Dict[str, Any]:
        """
    ...
    async def get_last_candles(self, symbol: str = DEFAULT_SYMBOL, market_type: MarketType = MarketType.SPOT, limit: int = 5) -> List[List[float]]:
        """
...
async def fetch_multiple_markets():
    ...
    for i, candle in enumerate(reversed(spot_candles), 1):
        ...
        print(f"  {i}. {timestamp}: O={candle[1]:.10f} H={candle[2]:.10f} L={candle[3]:.10f} C={candle[4]:.10f} V={base_volume:.0f} (${usdt_volume:,.2f})")
